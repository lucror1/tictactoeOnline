...
public class InboundServerHandler extends ChannelInboundHandlerAdapter {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        InboundServerHandler h = new InboundServerHandler(null);
        
        byte n = Settings.BOARD_WINNER_NULL;
        byte x = Settings.BOARD_WINNER_X;
        byte o = Settings.BOARD_WINNER_O;
        ...
        /* for (byte[] subboard : superBoard) {
            System.out.println(h.checkBoardForWinner(subboard));
        } */

        /* ByteBuf out = h.superBoardToByteBuf(superBoard);
        
        // Print the global winner
        System.out.println(out.readByte());

        // Print each subboard
        for (int i = 0; i < superBoard.length; i++) {
            for (int j = 0; j < superBoard[0].length + 1; j++) {
                System.out.print(out.readByte());
            }
            System.out.println();
        } */
    }
